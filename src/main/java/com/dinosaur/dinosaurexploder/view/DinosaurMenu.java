...
public class DinosaurMenu extends FXGLMenu {
  ...
  public DinosaurMenu() {
    ...
    languageManager
        ...
        .addListener((observable, oldValue, newValue) -> updateTexts());
    ...
    var title =
        FXGL.getUIFactoryService().newText(GameConstants.GAME_NAME, Color.LIME, FontType.MONO, 35);
        ...
        .addListener(
            (observable, oldValue, newValue) -> {
              AudioManager.getInstance()
                  .setVolume(newValue.doubleValue()); // <--- THIS LINE IS IMPORTANT
    ...
    try {
      ...
      InputStream menuImage =
          getClass().getClassLoader().getResourceAsStream("assets/textures/dinomenu.png");
      if (menuImage == null) {
      ...
      imageViewPlaying.setOnMouseClicked(
          mouseEvent -> {
  ...
  private static TranslateTransition getTranslateTransition(
      ImageView imageViewB, Image Background) {
    ...
    translateTransition.setToX(
        -Background.getWidth() + DinosaurGUI.WIDTH * 3.8); // Move to the left by the
    // width of the image
  ...
  public void onEnteredFrom(Scene prevState) {
    ...
    mainMenuSound.setMute(
        AudioManager.getInstance().isMuted()); // Optional: sync menu music with global mute
    mainMenuSound.setVolume(
        AudioManager.getInstance().getVolume()); // Optional: sync menu music with global volume
  }
}
