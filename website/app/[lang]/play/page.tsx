    ...
    <div className="max-w-6xl mx-auto w-full px-4 md:px-8 py-10">
      <div className="space-y-8">
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <article className="rounded-2xl border border-green-200 dark:border-green-500/40 bg-white/85 dark:bg-neutral-800/85 p-5 shadow-sm">
            <p className="font-mono text-xs uppercase tracking-[0.25em] text-green-700 dark:text-green-400">
              {playPage.officialEyebrow}
            </p>
            <h2 className="mt-2 font-retro text-2xl text-green-800 dark:text-green-200">
              {playPage.officialTitle}
            </h2>
            <p className="mt-3 font-mono text-green-950 dark:text-green-100 leading-relaxed">
              {playPage.officialDescription}
            </p>
            <div className="mt-4 flex flex-wrap gap-3">
              <a
                href={GAME_RELEASES_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="font-retro inline-flex items-center justify-center rounded-full bg-green-700 px-4 py-2 text-sm text-white transition hover:bg-green-600"
              >
                {playPage.downloadButton}
              </a>
              <a
                href={README_INSTALL_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="font-retro inline-flex items-center justify-center rounded-full border border-green-700 px-4 py-2 text-sm text-green-800 transition hover:bg-green-700 hover:text-white dark:border-green-500 dark:text-green-200 dark:hover:bg-green-600"
              >
                {playPage.readmeButton}
              </a>
            </div>
          </article>

          <article className="rounded-2xl border-2 border-green-700 dark:border-green-500 bg-black/85 p-5 shadow-sm">
            <p className="font-mono text-xs uppercase tracking-[0.25em] text-green-300">
              {playPage.demoEyebrow}
            </p>
            <h2 className="mt-2 font-retro text-2xl text-green-300 dark:text-green-400">
              {playPage.demoTitle}
            </h2>
            <p className="mt-3 font-mono text-green-100 dark:text-green-200 leading-relaxed">
              {playPage.demoDescription}
            </p>
          </article>
        </section>
