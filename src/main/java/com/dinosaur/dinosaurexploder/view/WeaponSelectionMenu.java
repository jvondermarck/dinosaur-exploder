...
public class WeaponSelectionMenu extends FXGLMenu {
  ...
  public WeaponSelectionMenu() {
    ...
    var title =
        FXGL.getUIFactoryService()
            .newText(
                languageManager.getTranslation("select_weapon"), Color.LIME, FontType.MONO, 35);
  ...
  private void showSelectionButton(double imageSize, int columns, GridPane weaponGrid) {
    ...
    for (int i = 1; i <= 3; i++) {
      Image weaponImage =
          new Image(
              Objects.requireNonNull(
                  getClass()
                      .getResourceAsStream(
                          "/assets/textures/projectiles/projectile"
                              + selectedShip
                              + "_"
                              + i
                              + ".png")));
      ...
      ImageView lockIcon =
          new ImageView(new Image(getClass().getResourceAsStream("/assets/textures/lock.png")));
      ...
      weaponButton.setOnAction(
          event -> {
            try {
              ...
              getDialogService()
                  .showBox(languageManager.getTranslation(exception.getMessage()), new VBox(), btn);
            }
          });
      weaponButton.setStyle(
      ...
      weaponButton.setOnMouseEntered(
          event -> {
